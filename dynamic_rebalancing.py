...
def run_simulation_dynamic(start_date, end_date, rebalancing_freq, num_periods_rebalancing, person, factor_model,
    ...
    while date < final_date:
        ...
        if days_since_rebalance >= min_days_between_rebalance:
            ...
            if action == 'rebalance' or date.strftime("%Y-%m-%d") in times:
                ...
                # ---------------- Evaluate last period  ----------------
                # Get actual asset returns for the period
                period_returns = get_period_returns(data, last_rebalance_date, period_start_time)

                # Calculate weighted portfolio return
                portfolio_return = np.matmul(period_returns, x)

                # Update portfolio
                port.add_period_returns(period_returns)
                port.add_cumulative_portfolio_return(portfolio_return)
                port.update_time(date)
    ...
    # Get actual asset returns for the last period
    period_returns = get_period_returns(data, last_rebalance_date, end_date)
...
def return_goal_actions(port, date, final_date, total_days, avg_regimes, risk_level, opt_method, person):
    ...
    # If portfolio return/annual return goal < 0.75, increase risk
        ...
        if risk_level >= 1 and avg_regimes[3] > 1.75:
...
def optimize_dynamic(port, data, person, factor_model, C, b, start_date, end_date, transaction_cost=True, first=False,
    ...
    if method == 'MVO':
        x = of.robust_mvo(mu, Sigma, lambda_, alpha, T, C, b, x_last=x_last, lb=lb, ub=ub, cardinality=cardinality,
                          transaction_penalty=0.01, include_trans_cost=transaction_cost, log_results=0, k_stock=20)

    if method == 'CVaR':
        x = of.cvar(alpha, returns, C, b, cardinality=cardinality, ub=ub, lb=lb, k_stock=20, log_results=0,
...
def compare_results(port, start_date, end_date, allocation=np.array([0.3, 0.7])):
    """
    Compare the portfolio wealth evolution with the 70/30 index
    ...
    # Plot the index return against the portfolio return
    plt.figure(figsize=(12, 6))
    plt.plot(dates, port.value, dates, index_value)
    plt.xticks(rotation=45)
    plt.xlabel("Time")
    plt.ylabel("Portfolio's Cumulative Return")
    plt.legend(["Portfolio Value", "70/30 Index Value"])
    plt.show()
